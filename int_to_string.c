...
int get_numbase_len(unsigned int base);
void fill_numbase_buff(unsigned int num, unsigned int base, char *buff, int buff_size);

/**
 * get_int - get a character pointer to new string containing int* @num: number to string*
 *
 ...
 */
 char *get_int(int num)
 {
     ...
     fill_numbase(temp, 10 , ret, length);
     if (num < 0 || num_1 < 0)
        ret[0] = '-';
  ...
  */
  ...
  /**
   * get_numbase - gets the length of buffer needed for an unsigned int.
                           ...
                           char *buff, int buff_size)
    {
        ...
        while (i >= 0)
        {
            ...
            if (rem > 9) /* return lowercase ascii value representation */
                buff[i] = rem + 87; /* 10 will be aa, 11 - b, ... */
            else
                buff[i] = rem + '0';
            num /= base;
            i--;
